...
<style>
.datatables-demo tbody tr td form {
		...
		<div class="board">
			...
			<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
				...
				<form name="listForm" action="<c:url value='/uat/uap/selectLoginPolicyList.do'/>" method="post">
					...
					<input type="hidden" name="emplyrId"> <input type="hidden" name="pageIndex" value="<c:if test="${empty loginPolicyVO.pageIndex }">1</c:if><c:if test="${!empty loginPolicyVO.pageIndex }"><c:out value='${loginPolicyVO.pageIndex}'/></c:if>">
						...
						<tr>
							<td>
								<form name="item" action="<c:url value='/uat/uap/getLoginPolicy.do'/>">
									...
									<span class="link ac">
										<input type="submit" value="<c:out value="${loginPolicy.emplyrId}"/>" onclick="fncSelectLoginPolicy('<c:out value="${loginPolicy.emplyrId}"/>'); return false;">
									</span>
								</form>
							</td>
							<td><c:out value="${loginPolicy.emplyrNm}" /></td>
							<td><c:out value="${loginPolicy.ipInfo}" /></td>
							<td><c:if test="${loginPolicy.lmttAt == 'Y'}">Y</c:if> <c:if
									test="${loginPolicy.lmttAt == 'N'}">N</c:if></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
