...
function fncRoleInsert(form) {
    ...
    if(confirm("<spring:message code="common.save.msg" />")){ //저장하시겠습니까?
        if(!validateRoleManage(form)){
        	...
        	var roleDc = $("#roleDc").val();
        	
        	var regex = /^\/+[a-zA-Z]+.do/;
        	var regex2 = /^\\+[a-zA-Z\/.*]/;
        	var ptn = form.rolePtn.value;
        	if(!regex.test(ptn) && !regex2.test(ptn)){
        		alert("롤 페턴을 확인해주세요 .\nex)/cmsco.do 형태로 입력해주세요.");
        		return;
        	}
...
<div class="wTableFrm">
	...
	<tbody>
		...
		<tr style="display: none;">
			<th>${title} <span class="pilsu">*</span></th>
			<td class="left">
				<select id="roleTyp" name="roleTyp">
	...
	<div class="btn">
		...
		<input type="submit" class="s_submit" style="margin-left:5px;" value="<spring:message code="button.create" />" title="<spring:message code="button.create" /> <spring:message code="input.button" />" /><!-- 등록 -->
