...
<body>
<%@include file="/WEB-INF/jsp/egovframework/com/head.jsp"%>
	<!-- content start -->
	<form name="cnfirmForm" method="post" action="">
		<input type="hidden" name="sbscrbTy" value="${sbscrbTy}">
		<!-- GPIN 실명인증 으로 가기위한 초기화값 -->
		<input type="hidden" name="realName" value="${realName}">
		<!-- 주민등록확인의 realname 변수와 구별  -->
		<!-- 실명인증을 재귀적으로 호출하는 경우 다음경로와 다음경로 버튼명을 셋팅함 -->
		<input type="hidden" name="nextUrl" value="${nextUrl}"> <input
			type="hidden" name="nextUrlName" value="${nextUrlName}">
		<table width="717" cellpadding="8" class="table-search" border="0">
			<tr>
				<td width="100%" class="title_left"><img
					src="<c:url value='/images/egovframework/com/cmm/icon/tit_icon.gif' />"
					width="16" height="16" hspace="3" align="middle" alt="제목아이콘이미지">&nbsp;주민등록
					실명확인</td>
			</tr>
		</table>
		<table width="717" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td>&nbsp;이름:<input type="text" name="realname"
					value="${realname}" title="이름입력"> &nbsp;주민등록번호 <input
					type="text" name="ihidnum" value="${ihidnum}" title="주민등록번호입력">(-없이
					13자리) <!-- 사용자유형 --> <input type="hidden" name="sbscrbTy_view"
					value="${sbscrbTy}"> <!-- 결과코드 --> <input type="hidden"
					name="result" value="${result}"> <input type="hidden"
					name="result_tmp" value="${result_tmp}"> <br> <c:if
			...
			<tr>
				<td>* 입력하신 주민등록번호는 실명인증을 하는데만 이용되며 저장되지 않습니다<br> * 개정
					"주민등록법"에 의해 타인의 주민등록번호를 부정사용하는 자는 3년 이하의 징역 또는 1천만원. 이하의 벌금이 부과될 수
					있습니다. 관련법률: 주민등록법 제37조(벌칙) 제9호(시행일 2006.09.24) 만약, 타인의 주민번호를 도용하여
					온라인 회원 가입을 하신 이용자분들은 지금 즉시 명의 도용을 중단하시길 바랍니다.
                    ...
                    <td align="left"><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_right.gif' />" width="8" height="20" alt="버튼이미지"></td>
                     -->
							<td><span class="button"><input type="submit"
									value="<spring:message code="button.realname" />"
									title="<spring:message code="button.realname" />"
									onclick="javascript:fnIhidnumCnfirm();return false;"></span></td>
							<td>&nbsp;</td>
							<td><span class="button"><input type="button"
									value="<spring:message code="button.moveToGpin" />"
									title="<spring:message code="button.moveToGpin" />"
									onclick="javascript:fnGpinMove();return false;"></span></td>
							<td>&nbsp;</td>
							<td><span class="button"><input type="button"
									value="<spring:message code="${nextUrlName}" />"
									title="<spring:message code="${nextUrlName}" />"
									onclick="javascript:fnNextMove();return false;"></span></td>
