...
function fncAddBannerInsert() {
    ...
    document.listForm.action = "<c:url value='/uss/ion/bnr/addViewBanner.do?bannerTarget=${bannerVO.bannerTarget}'/>";
    document.listForm.submit();
}
...
function linkPage(pageNo){
    document.listForm.searchCondition.value = "1";
    document.listForm.pageIndex.value = pageNo;
    <c:if test="${bannerVO.bannerTarget == 'main' }">
    	document.listForm.action = "<c:url value='/uss/ion/bnr/selectBannerList.do'/>";
    </c:if>
    <c:if test="${bannerVO.bannerTarget == 'middle' }">
		document.listForm.action = "<c:url value='/uss/ion/bnr/selectMiddleBannerList.do'/>";
	</c:if>
    document.listForm.submit();
}
...
<div class="board">
	<h1>
		<c:if test="${bannerVO.bannerTarget == 'main' }">	
		<spring:message code="ussIonBnr.mainbannerList.mainbannerList"/></h1><!-- 배너 관리 -->
		</c:if>
		<c:if test="${bannerVO.bannerTarget == 'middle' }">	
		<spring:message code="ussIonBnr.middlebannerList.middlebannerList"/></h1><!-- 배너 관리 -->
		</c:if>
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg"/>"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/uss/ion/bnr/addViewBanner.do'/>?bannerTarget=<c:out value="${bannerVO.bannerTarget}"/>" onclick="fncAddBannerInsert(); return false;" title="<spring:message code="button.create" />"><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
