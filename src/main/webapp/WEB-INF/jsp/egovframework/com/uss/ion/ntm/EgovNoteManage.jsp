...
<!-- 쪽지관리 -->
<c:set var="pageTitle">
	<spring:message code="comUssIonNtm.NoteManage.title" />
</c:set>
...
<validator:javascript formName="noteManage" staticJavascript="false"
...
<script type="text/javaScript">
	...
	function fn_egov_init_NoteManage() {
		...
		option
				.appendChild(document
						.createTextNode("수신:${noteManageMap.trnsmiterNm}(${noteManageMap.trnsmiterId})"));
	...
	function fn_egov_empList_NoteManage() {
		...
		for (var i = 0; i < FValue; i++) {
			if (document.getElementById(sbName).options[i].value != "") {
				sEmpList = sEmpList
						+ document.getElementById(sbName).options[i].value
						+ ",";
				sRecptnSeList = sRecptnSeList
						+ document.getElementById(sbName).options[i].recptnSe
						+ ",";
			}
	...
	function fn_egov_delete_NoteManage(nChk) {
		...
		if (nChk) {
			if (FValue == 0
					|| document.getElementById(sbName).selectedIndex == -1) {
...
<body onLoad="fn_egov_init_NoteManage();">
	...
	<div class="layout-container">
		...
		<form:form commandName="noteManage" name="noteManage"
			action="/uss/ion/ntm/registEgovNoteManage.do" method="post"
			...
			<div class="wTableFrm">
				...
				<table class="wTable"
					...
					<tbody>
						<tr>
							<th><img
								src="<c:url value='/images/egovframework/com/uss/ion/ntm/check.png'/>"
								alt="check" /><label for="noteSj"><spring:message
										code="comUssIonNtm.NoteMange.subject" /></label></th>
							<!-- 제목 -->
							<!-- 제목 -->
							<c:set var="subject">
								<spring:message code="comUssIonNtm.NoteManage.title" />
							</c:set>
							<td><form:input path="noteSj" title="${subject}"
						...
						<tr>
							<th><img
								src="<c:url value='/images/egovframework/com/uss/ion/ntm/check.png'/>"
								alt="check" /> <spring:message
									code="comUssIonNtm.NoteMange.receiver" /></th>
							<td>
								<div style="clear: both;"></div>
								<div style="text-align: left;">
									<!-- 수신 -->
									<c:set var="reception">
										<spring:message code="comUssIonNtm.NoteMange.reception" />
									</c:set>
									<!-- 참조 -->
									<c:set var="reference">
										<spring:message code="comUssIonNtm.NoteMange.reference" />
									</c:set>
									<!-- 목록 -->
									<c:set var="recptnEmpList">
										<spring:message code="comUssIonNtm.NoteMange.receiverList" />
									</c:set>
									<div>
										<select name="recptnEmp" title="${recptnEmnpList}"
											id="recptnEmp" style="width: 200px; height: 80px;" multiple>
											<option value=''></option>
										</select>
										<div class="recptn-btn">
											<!-- 수신/참조 선택  -->
											<form:radiobutton path="recptnSe" value="1"
												style="width:18px;border: 1px solid #dedede;" />
											<spring:message code="comUssIonNtm.NoteMange.reception" />
											<form:radiobutton path="recptnSe" value="2"
												style="width:18px;border: 1px solid #dedede;" />
											<spring:message code="comUssIonNtm.NoteMange.reference" />
											<form:errors path="recptnSe" cssClass="error" />
											<!-- 수신/참조자 선택 목록 팝업창 열기 -->
											<a
												href="<c:url value='/uss/ion/ntm/listEgovNoteEmpListPopup.do'/>"
												onClick="fn_egov_recptnEmpSearchPupup();return false;"><img
												src="<c:url value='/images/egovframework/com/cmm/btn/btn_search.gif'/>"
												align="middle" style="border: 0px; margin-right: 10px;"
												alt="${recptnEmnpList}" title="${recptnEmnpList}"></a>
											<!-- 수신자 목록 리스트 제외 -->
											<span class="btn_s"><a style="display: inline-block;background:#efb98d!important" href="#LINK"
												onClick="fn_egov_delete_NoteManage(1);"
												title="<spring:message code="comUssIonNtm.NoteMange.delete" /> <spring:message code="input.button" />"><spring:message
														code="comUssIonNtm.NoteMange.delete" /> </a></span>
										</div>
									</div>
								</div>
							</td>
						...
						<tr>
							...
							<th><img
								src="<c:url value='/images/egovframework/com/uss/ion/ntm/check.png'/>"
								alt="check" /> <spring:message
									code="comUssIonNtm.NoteMange.content" /></th>
							<td>
								<!-- 쪽지내용 --> <c:set var="contents">
									<spring:message code="comUssIonNtm.NoteMange.content" />
								</c:set> <form:textarea path="noteCn" name="noteCn" id="noteCn"
									title="${contents}" style="min-height:200px;width:100%" />
							</td>
						</tr>
					</tbody>
				</table>
				...
				<!-- 수신자목록리스트 -->
				<input type="hidden" name="recptnEmpList" id="recptnEmpList"
					value="">
				<!-- 수신자구분리스트 -->
				...
				<div class="btnNote">
					<input style="background:#75c4e6!important;" type="submit" class="s_submit"
						onClick="fn_egov_save_NoteManage(); return false;"
						value="<spring:message code="comUssIonNtm.NoteMange.send" />"
						title="<spring:message code="comUssIonNtm.NoteMange.send" /> <spring:message code="input.button" />" />
				</div>
				<div style="clear: both;"></div>
