...
<form name="listForm" id="listForm" action="<c:url value='/uss/umt/EgovMberManage.do'/>" method="post"> 
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
		<ul>
			...
			<li>
				...
				<span class="btn_c"><a href="javascript:userExcelDownload();">전체 엑셀다운로드</a></span>
...
<script type="text/javascript" src="/js/excel/excel/xlsx.full.min.js"></script>
<script type="text/javascript" src="/js/excel/file.save/FileSaver.min.js"></script>
...
function userExcelDownload() 
{
	...
	$.ajax({
		url: "/uss/umt/UserExcelDownload.do",
		type: "post",
		dataType: "json",
		//cache: false,
		//async: false,
		...
		if(isEmpty(res.data.lists) === true) {
			alert("출력할 데이터가 업습니다.");
			return;
		}*/
		
		var wb = XLSX.utils.book_new();
		wb.SheetNames.push("일반회원");
		
		//var wsData = [['아이디', '사용자이름', '별명', '사용자이메일', '생년월일', '등록일']];
		var wsData = [['아이디', '사용자이름', '별명', '사용자이메일', '생년월일', '등록일']];
		
		res.mberList.forEach(function(item) {
			
			
			var tempWsData = [];
			
			
			tempWsData[0] = item.userId;
			tempWsData[1] = item.userNm;
			tempWsData[2] = item.userNick;
			//tempWsData[3] = (item.user_gender == "M" ? "남" : "여");
			tempWsData[3] = item.emailAdres;
			tempWsData[4] = item.birthday;
			tempWsData[5] = item.sbscrbDe;
			
			wsData.push(tempWsData);
		});
	    ...
	    var wbout = XLSX.write(wb, {bookType:'xlsx',  type: 'binary'}); // 엑셀 파일 쓰기
	    saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), '일반회원.xlsx'); // 파일 다운로드
