...
<script type="text/javascript">
	...
	function fn_egov_inqire_menu() {
		...
		var width = 780;
		var height = 700;
		var url = "<c:url value='/sym/mnu/bmm/openPopup.do?requestUrl=/sym/mnu/bmm/selectMenuList.do'/>&width="
				+ (width - 20) + "&height=" + (height - 20);
		var openParam = "dialogWidth: " + width + "px; dialogHeight: " + height
				+ "px; resizable: 0, scroll: 1, center: 1";
...
<body>
	...
	<div class="layout-container">
		...
		<h2>
			<spring:message
				code="comSymMnuBmm.bkmkMenuManageRegist.pageTop.title" />
		</h2>
		<!-- 바로가기메뉴등록 -->

		<form name="bkmk" method="post"
			action="<c:url value='/sym/mnu/bmm/registBkmkInf.do'/>">
			<input type="hidden" name="menuId"
				value='<c:out value="${bkmkMenuManage.menuId}" />'>
			...
			<div class="wTableFrm">
				...
				<table class="wTable">
					...
					<tr>
						<th><spring:message
								code="comSymMnuBmm.bkmkMenuManageRegist.menuName" /> <span
							class="pilsu">*</span></th>
						<!-- 메뉴명 -->
						<td class="left"><input name="menuNm" type="text" size="30"
							value="<c:out value="${bkmkMenuManage.menuNm}" escapeXml="false" />"
							maxlength="60" readonly="readonly"
							class="readOnlyClass" title="메뉴명입력" /> <!-- <a href="javascript:fn_egov_inqire_menu();" style="selector-dummy: expression(this.hideFocus=false);"><img src="<c:url value='/images/egovframework/com/cmm/icon/search2.gif' />"
	     			style="vertical-align: middle" alt="<spring:message code="comSymMnuBmm.bkmkMenuManageRegist.selectMenu" />" title="<spring:message code="comSymMnuBmm.bkmkMenuManageRegist.selectMenu" />"></a> -->
							<!-- 메뉴선택 --> <a id="popupInqireMenu" title="메뉴선택"
							style="selector-dummy: expression(this.hideFocus = false);"><img
								...
								alt="<spring:message code="comSymMnuBmm.bkmkMenuManageRegist.selectMenu" />"
								title="<spring:message code="comSymMnuBmm.bkmkMenuManageRegist.selectMenu" />"></a>
						</td>
					</tr>
					<tr>
						<th><spring:message
								code="comSymMnuBmm.bkmkMenuManageRegist.menuURL" /> <span
							class="pilsu">*</span></th>
						<!-- 메뉴 URL -->
						<td class="left"><input name="progrmStrePath" type="text"
							size="30"
							value="<c:out value="${bkmkMenuManage.progrmStrePath}"/>"
							maxlength="90" style="width: 235px" readonly="readonly"
							class="readOnlyClass"
							title="<spring:message code="comSymMnuBmm.bkmkMenuManageRegist.enterMenuURL" />" />
						<!-- 메뉴URL입력 --></td>
					</tr>
				</table>
				...
				<div class="btn">
					<input class="s_submit" type="submit"
						...
						onclick="fn_egov_regist_bkmkInf(); return false;" />
					<!-- 저장 -->
					<span class="btn_s"><a
						href="<c:url value='/sym/mnu/bmm/selectBkmkMenuManageList.do'/>?pageIndex=<c:out value='${searchVO.pageIndex}'/>"
						onclick="fn_egov_select_bkmkInfs(); return false;"><spring:message
