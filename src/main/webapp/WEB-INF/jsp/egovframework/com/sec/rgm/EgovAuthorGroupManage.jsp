...
<c:set var="pageTitle">
	<spring:message code="comCopSecRgm.title" />
</c:set>
...
<script type="text/javaScript" language="javascript" defer="defer">
					...
					if (checkField[i].checked) {
						...
						if (returnId == "") {
							...
							returnAuthor = returnAuthor + ";"
									+ selectAuthor[i].value;
							returnRegYn = returnRegYn + ";"
									+ booleanRegYn[i].value;
							returnmberTyCode = returnmberTyCode + ";"
									+ listMberTyCode[i].value;
	...
	function fncSelectAuthorGroupPop() {
		...
		if (document.listForm.searchCondition.value == '3') {
			window.open("<c:url value='/sec/gmt/EgovGroupSearchList.do'/>",
							"notice",
							"height=500, width=485, top=50, left=20, scrollbars=no, resizable=no");
		} else {
...
.search_box input.s_btn:last-child {
...
<body>
	...
	<div class="layout-container">
		...
		<form:form name="listForm"
			action="${pageContext.request.contextPath}/sec/ram/EgovAuthorGroupList.do"
			method="post">
			<div class="board">
				<!-- 권한그룹관리 목록 -->
				<!-- 검색영역 -->
				<div class="search_box"
					...
					<ul>
						<!-- 조회조건 -->
						<li><select name="searchCondition"
							onchange="onSearchCondition()"
							title="<spring:message code="title.searchCondition" /> <spring:message code="input.cSelect" />">
								<option value="1"
									<c:if test="${authorGroupVO.searchCondition == '1'}">selected</c:if>><spring:message
										code="comCopSecRgm.searchCondition.userId" /></option>
								<!-- 사용자 ID -->
								<option value="2"
									<c:if test="${authorGroupVO.searchCondition == '2'}">selected</c:if>><spring:message
										code="comCopSecRgm.searchCondition.userNm" /></option>
								<!-- 사용자 명- -->
								<option value="3"
									<c:if test="${authorGroupVO.searchCondition == '3'}">selected</c:if>><spring:message
										code="comCopSecRgm.searchCondition.group" /></option>
								<!-- 그룹 -->
						</select></li>
						<!-- 검색키워드 및 조회버튼 -->
						<li><input class="s_input" name="searchKeyword" type="text"
							...
							maxlength="155"> <input type="button" class="s_btn"
							onClick="fncSelectAuthorGroupPop()"
							value="<spring:message code="comCopSecRgm.btn.groupInquire" />"
							title="<spring:message code="comCopSecRgm.btn.groupInquire" /> <spring:message code="input.button" />" />
						<!-- 그룹조회팝업 --> <input type="button" class="s_btn"
							onClick="fncSelectAuthorGroupList('1')"
							value="<spring:message code="button.inquire" />"
							title="<spring:message code="button.inquire" /> <spring:message code="input.button" />" />
						<!-- 조회 --> <input type="button" class="s_btn"
							onClick="fncAuthorGroupDeleteList()"
							value="<spring:message code="title.delete" />"
							title="<spring:message code="title.delete" /> <spring:message code="input.button" />" />
						<!-- 삭제 --> <input type="button" class="s_btn"
							onClick="fncAddAuthorGroupInsert()"
							value="<spring:message code="button.create" />"
							title="<spring:message code="button.create" /> <spring:message code="button.create" />" />
						<!-- 등록 --></li>
					</ul>
				</div>
					...
					<thead>
						<tr>
							...
							<th class="board_th_link"><spring:message
									code="comCopSecRgm.list.userId" /></th>
							<!-- 사용자 ID -->
							<th><spring:message code="comCopSecRgm.list.userNm" /></th>
							<!-- 사용자 명 -->
							<th><spring:message code="comCopSecRgm.list.userType" /></th>
							<!-- 사용자 유형 -->
							<th><spring:message code="comCopSecRgm.list.author" /></th>
							<!-- 권한 -->
							<th><spring:message code="comCopSecRgm.list.regYn" /></th>
							<!--등록 여부 -->
						</tr>
					</thead>
					<tbody class="ov">
						...
						<c:forEach var="authorGroup" items="${authorGroupList}"
							varStatus="status">
							<tr>
								<td><input type="checkbox" name="delYn" class="check2"
									title="선택"><input type="hidden" name="checkId"
									value="<c:out value="${authorGroup.uniqId}"/>" /></td>
								<td><c:out value="${authorGroup.userId}" /></td>
								<td><c:out value="${authorGroup.userNm}" /></td>
								<td><c:out value="${authorGroup.mberTyNm}" /><input
									type="hidden" name="mberTyCode"
									value="${authorGroup.mberTyCode}" /></td>
								<td><select name="authorManageCombo"
									title="<spring:message code="comCopSecRgm.list.authorManageCombo" />">
										<!-- 권한선택 -->
										<c:forEach var="authorManage" items="${authorManageList}"
											varStatus="status">
											<option value="<c:out value="${authorManage.authorCode}"/>"
												<c:if test="${authorManage.authorCode == authorGroup.authorCode}">selected</c:if>><c:out
													value="${authorManage.authorNm}" /></option>
										</c:forEach>
								</select></td>
								<td><c:out value="${authorGroup.regYn}" /><input
									type="hidden" name="regYn"
									value="<c:out value="${authorGroup.regYn}"/>"></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
			...
			<input type="hidden" name="pageIndex"
				value="<c:out value='${authorGroupVO.pageIndex}'/>" />
		</form:form>
	</div>
