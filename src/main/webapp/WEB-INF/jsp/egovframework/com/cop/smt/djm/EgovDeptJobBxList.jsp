...
<body onLoad="fn_egov_init_deptjobbx()">
	...
	<div class="layout-container">
		...
		<div class="board">
			<h1>
				<spring:message code="comCopSmtDjm.deptJobBxList.title" />
			</h1>
			<!-- 부서업무함관리 목록조회 -->

			<form name="frm" method="post"
				action="<c:url value='/cop/smt/djm/selectDeptJobBxList.do'/>">
				<input name="pageIndex" type="hidden"
					value="<c:out value='${searchVO.pageIndex}'/>"> <input
					type="hidden" name="deptJobBxId"> <input type="hidden"
					name="ordrCnd"> <input type="hidden" name="deptId">
				<input type="hidden" name="indictOrdr" value="0"> <input
					type="hidden" name="checkedDeptJobBxId"
					value="<c:out value='${searchVO.deptJobBxId}'/>">

				<div class="search_box"
					title="<spring:message code="common.searchCondition.msg" />">
					<ul>
						<li><label for=""><spring:message
									code="select.searchCondition" />&nbsp; : &nbsp;</label>
						<!-- 조회조건선택 --> <select class="select" name="searchCnd"
							title="<spring:message code="select.searchCondition" />">
								...
								<option value="0"
									<c:if test="${searchVO.searchCnd == '0'}">selected="selected"</c:if>><spring:message
										code="comCopSmtDjm.deptJobBxList.deptNm" /></option>
								<!-- 부서명 -->
								<option value="1"
									<c:if test="${searchVO.searchCnd == '1'}">selected="selected"</c:if>><spring:message
										code="comCopSmtDjm.deptJobBxList.deptJobNm" /></option>
								<!-- 부서업무함명 -->
						</select> <input class="s_input2 vat" type="text" name="searchWrd"
							value='<c:out value="${searchVO.searchWrd}"/>' size="15"
							onkeypress="press(event);"
							title="<spring:message code="title.search" /> <spring:message code="input.input" />" />
						<!-- 검색어 입력 --> <input class="s_btn" type="submit"
							value="<spring:message code="title.inquire" />"
							title="<spring:message code="title.inquire" />"
							onclick="fn_egov_select_deptjobbx('1'); return false;" />
						<!-- 조회 --> <input class="s_btn" type="submit"
							value="<spring:message code="button.create" />"
							title="<spring:message code="button.create" />"
							onclick="fn_egov_insert_deptjobbx('1'); return false;" /></li>
					</ul>
				</div>
			</form>
			...
			<div class="c_box">
				<table
					...
					<caption>
						<spring:message code="comCopSmtDjm.deptJobBxList.deptJobBxManage" />
					</caption>
					<!-- 부서업무함관리 -->
					<colgroup>
						<col style="width: 5%" />
					...
					<thead>
						<tr>
							...
							<th scope="col"><spring:message
									code="comCopSmtDjm.deptJobBxList.deptNm" /></th>
							<!-- 부서명 -->
							<th scope="col"><spring:message
									code="comCopSmtDjm.deptJobBxList.deptJobNm" /></th>
							<!-- 부서업무함명 -->
							<th scope="col"><spring:message
									code="comCopSmtDjm.deptJobBxList.finalEditor" /></th>
							<!-- 최종수정자 -->
							<th scope="col"><spring:message
									code="comCopSmtDjm.deptJobBxList.finalEditDate" /></th>
							<!-- 최종수정일 -->
						</tr>
					</thead>
					<tbody>
						<c:forEach var="result" items="${resultList}" varStatus="status">
							<tr>
								<td>
									<form name="deptJobBxCheck" method="post"
										action="<c:url value='/cop/smt/djm/selectDeptJobBxList.do'/>">
										<input type="radio" name="check1"
											id="<c:out value="${result.deptJobBxId}"/>::<c:out value="${result.indictOrdr}"/>::<c:out value="${result.deptId}"/>"
											class="check2"
											onclick="fCheck('<c:out value="${status.count}"/>', '<c:out value="${result.deptJobBxId}"/>','<c:out value="${result.indictOrdr}"/>','<c:out value="${result.deptId}"/>')"
											title="선택"> <input type="submit" value=""
											style="border: 0px; height: 0px; color: #f7f7f7; display: none">
									</form>
								</td>
								<td><c:out
								...
								<td>
									<form name="deptJobBxVO" method="post"
										action="<c:url value='/cop/smt/djm/modifyDeptJobBx.do'/>">
										<input name="pageIndex" type="hidden"
											value="<c:out value='${searchVO.pageIndex}'/>"> <input
											name="searchCnd" type="hidden"
											value="<c:out value='${searchVO.searchCnd}'/>"> <input
											name="searchWrd" type="hidden"
											value="<c:out value='${searchVO.searchWrd}'/>"> <input
											type="hidden" name="deptJobBxId"
											value="<c:out value="${result.deptJobBxId}"/>"> <span
											class="link"><input type="submit"
											value="<c:out value="${result.deptJobBxNm}"/>"
											onclick="javascript:fn_egov_inqire_deptjobbx('<c:out value="${result.deptJobBxId}"/>'); return false;"
											style="text-align: left;"></span>
									</form>
								</td>
								<td><c:out value="${result.lastUpdusrNm}" /></td>
								<td><c:out
										value="${fn:substring(result.lastUpdusrPnttm, 0, 10)}" /></td>
							</tr>
						</c:forEach>
				...
				<div class="btn">
					<a href="#LINK"
						onclick="fn_egov_inqire_deptjobbx_ordr('up'); return false;">▲</a>
					<a href="#LINK"
						onclick="fn_egov_inqire_deptjobbx_ordr('down'); return false;">▼</a>
				</div>
			</div>
			...
			<div class="pagination">
				<ul>
					<ui:pagination paginationInfo="${paginationInfo}" type="image"
						jsFunction="fn_egov_select_deptjobbx" />
				</ul>
			</div>
		</div>
