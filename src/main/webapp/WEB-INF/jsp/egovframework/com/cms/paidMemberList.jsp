...
function paidMemberView(id){
	document.listForm.selectedId.value = id;
    document.listForm.action = "/paidMemberView.do";
...
<form id="listForm" name="listForm" action="<c:url value='/paidMemberView.do'/>" method="post"> 
	<div class="board">
		<!-- 검색영역 -->
			...
			<c:forEach var="result" items="${resultList}" varStatus="status">
				<tr>
				    ...
				    <td>
				        ...
				        <input name="checkId" type="hidden" value="<c:out value='${result.idx}'/>"/>
				    </td>
				    
				    <td><a href="javascript:paidMemberView('<c:out value="${result.idx}"/>')"><c:out value="${result.mberId}"/></a></td>
				    <td><c:out value="${result.name}"/></td>
				    <td><c:out value="${result.phone}"/></td>
				    <td><c:out value="${result.mberEmailAdres}"/></td>
				    <td><c:out value="${fn:substring(result.regiDate,0,10)}"/></td>
				    <td>
				    	<c:choose>
				    		<c:when test="${result.dues == '1'}">
				    			1년 회원
					    	</c:when>
					    	<c:otherwise>
					    		평생 회원
				    ...
				    <td>
				    	<c:choose>
				    		<c:when test="${result.mberStatus == 'I'}">
				    			승인대기
					    	</c:when>
					    	<c:when test="${result.mberStatus == 'C'}">
				    			승인완료
					    	</c:when>
					    	<c:otherwise>
					    		승인거부
