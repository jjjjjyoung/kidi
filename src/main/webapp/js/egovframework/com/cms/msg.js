...
function setDate() {
    var date = new Date();
    document.sms.R_YEAR.value = date.getFullYear();
    document.sms.R_MONTH.value = fillString(date.getMonth()+1, 2, '0');
    document.sms.R_DAY.value = fillString(date.getDate(), 2, '0');
    document.sms.R_HOUR.value = fillString(date.getHours(), 2, '0');
    document.sms.R_MIN.value = fillString(date.getMinutes(), 2, '0');
}
 ...
 */ 
function add(str) {
    document.sms.strData.focus();
    document.sms.strData.value+=str; 
    ChkLen();
    return;
}
...
function ChkLen() {
    ...
    var len = sms.strData.value.length;
    for (i=0;i<len;i++){
        pos = sms.strData.value.charAt(i);
    ...
    if(isNewSubject && msglen <= 30) sms.strSubject.value = sms.strData.value;

    sms.strDataCount.value = msglen;
 ...
 */ 
function addItem() {
    if (sms.addCallNum.value == "" ) {
        alert("받는분 번호를 정확히 입력하시고 추가해주세요.");
        sms.addCallNum.value = "";
        sms.addCallNum.focus();
        return;
    } 

    if (!isInteger(sms.addCallNum.value, 1)) {
...
function addSelectBox(name, tel) {
    ...
    index = sms.strDest.options.length;
    ...
    if(phoneRe.exec(tel)){
        ...
        for (i=0; i < index ; i++) {
            if (sms.strDest.options[i].text == tel) {
                alert(name+"님은 이미 수신자에 포함되어 있습니다.");
                return;
            }
        }

        sms.strDest.options.length = index+1;
        sms.strDest.options[index].value = tel +";";
        sms.strDest.options[index].text = tel;
        sms.strDest.options.length = index+1;
    } 
}   
...
function CWCheck(form) {
    ...
    if (document.sms.chkSendFlag[0].checked) {    // 즉시전송을 선택했을 경우
      */  
        sDest.style.display = "none";
        document.sms.R_YEAR.disabled = true;
        document.sms.R_MONTH.disabled = true;
        document.sms.R_DAY.disabled = true;
        document.sms.R_HOUR.disabled = true;
        document.sms.R_MIN.disabled = true;
        /*
    }
    else                                            // 예약전송을 선택했을 경우
    {
        sDest.style.display = "block";    
        document.sms.R_YEAR.disabled = false;
        document.sms.R_MONTH.disabled = false;
        document.sms.R_DAY.disabled = false;
        document.sms.R_HOUR.disabled = false;
        document.sms.R_MIN.disabled = false;
        document.sms.R_YEAR.focus();
    }*/
    document.sms.strTelList.value = "";
...
function varcheck() {
    ...
    // 일괄 문구 치환
    sms.strData.value = sms.strData.value.replace(/\u200b/g, '');

    var varStrData = document.sms.strData.value;
    var varStrSubject = document.sms.strSubject.value;
    var varStrCallBack = document.sms.strCallBack.value;
    ...
    if (document.sms.strDest.length < 2){
        alert("수신자 번호를 입력해 주세요");
        document.sms.addCallNum.focus();
        return false;
    }
    ...
    if (varStrCallBack  == null || varStrCallBack == ""){
        alert("회신번호를 입력하세요");
        document.sms.strCallBack.focus();
        return false;
    }
    ...
    document.sms.strTelList.value = "";
    for (k=1; k < document.sms.strDest.length; k++) { 
        document.sms.strTelList.value += document.sms.strDest.options[k].value;
    }
